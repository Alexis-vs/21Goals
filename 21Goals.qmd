...
get_player_informations <- function(player_link){
  
  goal_link <- get_goal_link(player_link)
...
df_informations <- lapply(data$link, get_player_informations)
...
df_group_by <- data %>%
  group_by(player) %>%
  summarise(sum_goals = sum(n_goals))
...
reactable(
  ...
  columns = list(
    ...
    n_goals = colDef(name = "Goals", cell = function(value){
      width <- paste0(value / max(data$n_goals) * 100, "%")
