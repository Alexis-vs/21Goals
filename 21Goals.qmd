...
```{r}
df_informations <- lapply(unique(data$round_goal_link), get_goals_by_rounds)
df_informations <- do.call("rbind", df_informations)
...
# Création du graphique line chart
highchart() %>%
  hc_chart(type = "line") %>%
  hc_title(text = "Cumultaive Goals by Matchweek") %>%
  hc_xAxis(categories = paste("Round", sort(unique(df$Round)))) %>%
