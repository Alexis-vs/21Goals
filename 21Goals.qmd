...
```{r}
data <- xlsx::read.xlsx("data.xlsx", sheetIndex = 1) %>% head(8)
```
...
get_img_player <- function(page){
  ...
  if(is_empty(img)){img <- "images/mbappe-chute.jpg"}
  
  return(img)
  
}

get_player_informations <- function(player_link, season){
  
  goal_link <- get_dom_goal_link(player_link)
...
```{r}
df_informations <- lapply(data$link, get_player_informations, season = "2024-2025")
df_informations <- do.call("rbind", df_informations)

data <- cbind(data, df_informations)
```
